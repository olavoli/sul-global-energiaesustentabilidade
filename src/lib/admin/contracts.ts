...
export function validOperationalId(value: string): boolean {
  return /^(?:(?:inbox|decision|cluster|translation|run|pitch|evidence|claim|radar|graph|sgr|duplicate|trend)-[a-f0-9]{16}|concept-relation-[a-f0-9]{16}|evidence-(?:dossier|source|claim|item)-[a-f0-9]{16})$/.test(
    value,
  );
}
